...
class Nodo:
    ...
    def get_proxima_posicao_vazio(self, acao):
        if acao == Direcao.acima:
            return self.posicao_vazio - 3 if self.posicao_vazio > 2 else self.posicao_vazio
        elif acao == Direcao.abaixo:
            return self.posicao_vazio + 3 if self.posicao_vazio < 6 else self.posicao_vazio
        elif acao == Direcao.esquerda:
            return (
                self.posicao_vazio - 1
                if self.posicao_vazio not in (0, 3, 6)
                else self.posicao_vazio
            )
        else:
            return (
                self.posicao_vazio + 1
                if self.posicao_vazio not in (2, 5, 8)
                else self.posicao_vazio
            )
    ...
    def faz_acao(self, acao):
        ...
        return Nodo(estado_filho, self, acao.name, self.custo_caminho+1)
